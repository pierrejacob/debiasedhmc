...
MatrixXd HmcSampler::sampleNext(bool returnTrace ) {
  Rcpp::RNGScope scope;
    ...
    while(2){
        ...
        double velsign =0;
        Rcpp::NumericVector normals = Rcpp::rnorm(dim);
...
MatrixXd HmcSampler::sampleNext_given_velocity(const VectorXd & velocity, bool returnTrace ) {
    ...
    while (1){
      ...
      if (t==0 || tt < t){                 // if no wall to be hit (t==0) or not enough time left to hit the wall (tt<t2)
        ...
        if (velsign <0 ){
          newvel = true;
          break ;
        }    //get out of while(1). resample the velocity and start again. this occurs rarely, due to numerical instabilities
      }
...
double HmcSampler::_verifyConstraints(const VectorXd & b){
  ...
  for (int i=0; i != linearConstraints.size(); i++ ){
    ...
    // std::cerr << "verifying constraints... found " << check << std::endl;
    if (i==0 || check < r) {
      r = check;
    }
  }
