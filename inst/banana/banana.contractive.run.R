...
## achtung the following coupling is of Normal(0, I) with Normal(0, I)
## not a coupling of Normal(mu1, I) with Normal(mu2, I)
contr_coupling  <- function(mu1, mu2, gamma = 1){
  d <- length(mu1)
  xi <- rnorm(d, 0, 1)
  eta <- rep(0, d)
  # define z and e
  ...
  #
  utilde <- runif(1, 0, 1)
  if (log(utilde) < (dnorm(sum(e * xi) + gamma * normz, 0, 1, log = TRUE) - dnorm(sum(e*xi), log = TRUE))){
    eta <- xi + gamma * z
  } else {
    eta <- xi - 2 * (sum(e * xi)) * e
  }
...
hmc_contr_coupled_kernel <- function(chain_state1, chain_state2, current_pdf1, current_pdf2, nsteps, stepsize){
  xy <- contr_coupling(chain_state1, chain_state2, gamma = 1)
  current_v1 <- xy[,1]
  current_v2 <- xy[,2]
  leapfrog_result1 <- leapfrog(chain_state1, current_v1, nsteps, stepsize)
  leapfrog_result2 <- leapfrog(chain_state2, current_v2, nsteps, stepsize)
  ...
  # the acceptance ratio also features the "kinetic energy" term of the extended target
  accept_ratio1 <- accept_ratio1 + sum(current_v1^2) / 2 - sum(proposed_v1^2) / 2
  accept_ratio2 <- accept_ratio2 + sum(current_v2^2) / 2 - sum(proposed_v2^2) / 2
...
samplemeetingtime <- function(omega, mh_stepsize, hmc_stepsize, hmc_nsteps, max_iterations = Inf){
  ...
  mixture_coupled_kernel <- function(chain_state1, chain_state2, current_pdf1, current_pdf2){
    if (runif(1) < omega){
      ...
      return(hmc_contr_coupled_kernel(chain_state1, chain_state2, current_pdf1, current_pdf2, hmc_nsteps, hmc_stepsize))
    }
  }
  #
...
save(results.df, nrep, omega, hmc_nsteps, hmc_stepsize, mh_stepsize, file = "banana.contr.meetings.RData")
load(file = "banana.contr.meetings.RData")
