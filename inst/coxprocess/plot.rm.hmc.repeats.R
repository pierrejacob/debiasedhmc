...
for (ijob in 1:njobs){
  filename <- paste("inst/coxprocess/output/output.rm.hmc.repeat", ijob, ".RData", sep = "")
  load(file = filename)
  for (irep in 1:nreps){
...
# load optimal RM-HMC asymptotic variance
load("inst/coxprocess/output/optimal.rm.hmc.RData")
