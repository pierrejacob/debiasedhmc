...
setmytheme()
set.seed(1)
# scaling of HMC on standard Normal target in arbitrary dimension
...
#
samplemeetingtime <- function(omega, mh_stepsize, hmcconstant, dimension, max_iterations = Inf){
  # HMC parameters
...
tic()
omega <- 0.05
hmcconstants <- c(1, 1.5, 2)
mh_stepsize <- 1e-3
nrep <- 100  ## replace this by 1000 to get figure of the paper
dimensions <- floor(seq(from = 1000, to = 10000, length.out = 10))
results.df <- data.frame()
for (hmcconstant in hmcconstants){
  for (dimension in dimensions){
    ...
    results_ <- foreach(irep = 1:nrep) %dorng% {
      samplemeetingtime(omega, mh_stepsize, hmcconstant, dimension, max_iterations = Inf)
    }
    meetingtimes <- sapply(results_, function(x) x$meetingtime)
    results.df <- rbind(results.df, data.frame(nsteps = hmc_nsteps,
                                               constant = hmcconstant,
                                               dimension = dimension,
...
save(results.df, file = "scaling.hmc.meetings.v3.RData")
