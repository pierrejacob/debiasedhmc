...
#'@export
rtmgR <- function(n, M, r, initial, f = NULL, g=NULL, q=NULL, burn.in=30){
  ...
  # samples = .Call( "rtmg", n+burn.in, seed,  initial2, numlin, f2, g2, numquad, quads, PACKAGE = "debiasedhmc")
  if (is.null(f2)) f2 <- matrix()
  if (is.null(g2)) g2 <- vector()
  if (is.null(quads)) quads <- matrix()
  samples = rtmg(n+burn.in, seed, initial2, numlin, f2, g2, numquad, quads)
  # samples = .Call( "rtmg", n+burn.in, seed,  initial2, numlin, f2, g2, numquad, quads)
